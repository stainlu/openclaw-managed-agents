...
- **`defineSingleProviderPluginEntry` auto-register** — would eliminate the `apply-provider-config.mjs` bootstrap path for Category B providers (currently moonshot + deepseek; likely qwen in the future).
