...
- **Public ingress on port 8080.** The orchestrator is reachable from the public internet via the firewall rule. The existing `OPENCLAW_GATEWAY_TOKEN` Bearer auth is the only gate in the default setup. For production, either (a) restrict the `source-ranges` on the firewall rule to specific client CIDRs, (b) front with an HTTPS Load Balancer + Cloud Armor (native GCP path), or (c) put the orchestrator behind a Cloudflare Tunnel (no ports exposed, authenticated access only).
