...
- **SSH is a break-glass hatch, not an operator UX.** An earlier deploy-hetzner.sh version added an `ssh.socket` override to open port 222 as an ISP fallback, a fail2ban whitelist for proxy exits, and an IPv6/IPv4 dual-stack workaround — each of which introduced its own failure mode. The right fix was to stop treating SSH as part of routine ops: the HTTP API on :8080 is the only interface the operator should need. If the VM is wedged, destroy + redeploy. Cloud Run and Fargate don't expose SSH at all, making this trap structurally impossible there.
