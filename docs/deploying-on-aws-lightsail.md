...
**The deploy uses pre-built images from GHCR** — `ghcr.io/stainlu/openclaw-managed-agents-{orchestrator,agent}:latest`, published by `.github/workflows/publish-images.yaml` on every push to `main`. Deploy time dropped from ~12 min (earlier build-from-source baseline) to ~5 min because `npm install -g openclaw@2026.4.24` (6.5 min of the original 12) no longer runs on the target VM. CPU burst credits also stay full for the first turn, shortening cold-spawn time as a side effect.
